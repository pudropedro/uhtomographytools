...
%CREATE_CT_OPERATOR_2D_FAN_ASTRA Create CT forward model.
%   create_ct_operator_2d_fan_astra( ctData, xDim, yDim ) computes the 
%   forward model, i.e. X-ray projection operator, for the fan-beam CT data
%   given in ctData. The x- and y-dimensions of the CT volume are given by 
%   xDim and yDim, respectively. The imaging geometry is created using the 
%   metadata in ctData. It is assumed that a flat detector has been used 
...
%   Spot Linear-Operator Toolbox have been added to the MATLAB path. The
%   operator is computed using the CPU.
%
