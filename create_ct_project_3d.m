function [ ctData ] = create_ct_project_3d( projectName, varargin )
%CREATE_CT_PROJECT_3D Prepare 3D tomography data for reconstruction.
%   ctData = create_ct_project_3d(projectName) creates a data structure 
%   which contains the 3D sinogram and imaging parameters of a CT scan. A
...
%
%   ctData = create_ct_project_3d(projectName, argumentName, argumentValue)
...
rows            = rowsRaw / binning;
cols            = colsRaw / binning;
...
parameters.projectionRows       = rows;
parameters.projectionCols       = cols;
...
% Initialize empty 3D sinogram using ASTRA geometry
% Data type 'single' is used due to memory considerations
sinogram = zeros(cols, parameters.numberImages, rows, 'single');

% Read each image, compute log transform, and place into 3D sinogram
...
if saveToDisk == 1
    ...
    if strcmp(outputFilename, '')
        ...
        outputFilename      = strcat(projectName, '_ct_project_3d', binningString);
    end
